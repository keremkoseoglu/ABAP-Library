CLASS zcl_bc_tcode_list DEFINITION
  ...
  PUBLIC SECTION.
    ...
    METHODS get_data
      RETURNING
        VALUE(rt_tcode_list) TYPE tt_tcode_list.
...
CLASS ZCL_BC_TCODE_LIST IMPLEMENTATION.
  ...
  METHOD read_requests.
    ...
    gt_trans = corresponding #(
      zcl_Bc_transport_request=>get_request_and_objects(
        it_tag = value #( for ls_ret in gt_ret (
                   pgmid    = c_pgmid_r3tr
    ...
    LOOP AT gt_ret ASSIGNING FIELD-SYMBOL(<ls_ret>).
      ...
      LOOP AT gt_trans ASSIGNING FIELD-SYMBOL(<ls_trans1>) WHERE obj_name EQ <ls_ret>-tcode.
      ...
      IF sy-subrc EQ 0 AND <ls_ret>-trkorr IS INITIAL.
