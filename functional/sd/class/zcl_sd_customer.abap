CLASS zcl_sd_customer DEFINITION
  ...
  PUBLIC SECTION.
    ...
    METHODS is_employer
      RETURNING
        VALUE(rv_employer) TYPE abap_bool .
    METHODS get_land
      RETURNING
        VALUE(output) TYPE REF TO zcl_bc_land
      RAISING
...
CLASS ZCL_SD_CUSTOMER IMPLEMENTATION.
  ...
  METHOD cache_name1.
    ...
    DATA:
      lt_kunnr_rng TYPE trty_kunnr_range.
  ...
  METHOD get_invoice_scope_by_tax.
    ...
    IF sy-subrc <> 0.
      ...
      IF iv_purpose <> zcl_sd_customer=>c_purpose-mutabakat AND ls_cache-fatura_kapsami <> c_fatura_kapsami-e_fatura.
        ...
        ls_cache-fatura_kapsami = COND #(
          WHEN lv_earc = abap_true THEN c_fatura_kapsami-e_arsiv
  ...
  METHOD ensure_defined_in_dist_channel.
    ...
    IF sy-subrc <> 0.
      ...
      SELECT SINGLE FROM knvv                           "#EC CI_NOORDER
             FIELDS loevm
             WHERE  kunnr = @gs_def-kunnr   AND
                    vkorg = @ls_cache-vkorg AND
                    vtweg = @ls_cache-vtweg
             INTO  @DATA(loevm).

      ls_cache-error = COND #( WHEN sy-subrc <> 0
                               THEN NEW #( textid     = zcx_sd_customer_def=>undefined_in_sd_area
                                           kunnr      = gs_def-kunnr
                                           cust_keys  = |{ ls_cache-vkorg } { ls_cache-vtweg }| )

                               WHEN sy-subrc = 0 AND loevm = abap_true
                               THEN NEW #( textid     = zcx_sd_customer_def=>deleted_in_sd_area
                                           kunnr      = gs_def-kunnr
                                           cust_keys  = |{ ls_cache-vkorg } { ls_cache-vtweg }| ) ).
