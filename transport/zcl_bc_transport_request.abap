...
CLASS zcl_bc_transport_request IMPLEMENTATION.
  ...
  METHOD add_objects_from_request.
    TRY.
        me->core->add_objects_from_request(
            from              = VALUE #( FOR _from IN it_from ( _from-trkorr ) )
            wait              = iv_wait
            sort_and_compress = iv_sort_and_compress ).
  ...
  METHOD get_empty_open_requests.
    TRY.
        rt_list = ycl_addict_transport_request=>get_empty_open_requests( CORRESPONDING #( it_trkorr_rng ) ).
  ...
  METHOD get_instance.
    ...
    IF sy-subrc <> 0.
      TRY.
          DATA(addict) = ycl_addict_transport_request=>get_instance(
              trkorr = iv_trkorr
              top    = iv_top ).
  ...
  METHOD get_modified_objects.
    rt_obj = CORRESPONDING #(
        ycl_addict_transport_request=>get_modified_objects(
          CORRESPONDING #( it_obj ) ) ).
  ENDMETHOD.
  ...
  METHOD get_obj_related_requests.
    TRY.
        rt_list = CORRESPONDING #(
            me->core->get_obj_related_requests(
                include_self   = iv_include_self
                top            = iv_top
                obj_name_rng   = CORRESPONDING #( it_obj_name_rng )
  ...
  METHOD get_open_requests.
    rt_list = CORRESPONDING #(
        ycl_addict_transport_request=>get_open_requests(
            as4text_rng       = CORRESPONDING #( it_as4text_rng )
            trfunction_rng    = CORRESPONDING #( it_trfunction_rng )
  ...
  METHOD get_request_and_objects.
    rt_object = CORRESPONDING #(
        ycl_addict_transport_request=>get_request_and_objects(
          CORRESPONDING #( it_tag ) ) ).
  ENDMETHOD.
  ...
  METHOD get_request_objects.
    ycl_addict_transport_request=>get_request_objects(
      EXPORTING trkorr_rng    = CORRESPONDING #( it_trkorr_rng )
                pgmid_rng     = CORRESPONDING #( it_pgmid_rng )
  ...
  METHOD get_subtasks.
    TRY.
        rt_subtask = VALUE #(
          FOR _st IN me->core->get_subtasks( iv_only_empty )
          ( trkorr = _st-trkorr
  ...
  METHOD release.
    TRY.
        me->core->release(
          EXPORTING rel_subtasks_too          = iv_rel_subtasks_too
